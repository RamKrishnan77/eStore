...
import { Container,Row,Col,Image,Badge, Button, ButtonGroup, Carousel } from 'react-bootstrap';
import '../App.css'
...
const ProductDetailsPage = (props) => {
    const productObject = props.products.data.filter((ob)=> ob.id == props.match.params.productId)[0]
    const [matches,setMatches] = useState({matches: window.matchMedia("(min-width: 768px)").matches})
        ...
        {matches.matches && (
            ...
            <Container className='product-details-container-info' fluid>
                    <Col md={12} className='product-desc-row-1'>
                            ...
                            <Row>
                                <Col><h2>{productObject.name}</h2></Col>
                            </Row>
                            <Row>
                                <Col><Badge variant='secondary'>{productObject.rating}</Badge>
                                <Badge variant='secondary'>{productObject.gender}</Badge>
                                <Badge variant='secondary'>{productObject.type}</Badge>
                                </Col>
                            </Row>
                            <Row className>
                                <Col className="price-row"><h3>Rs. {productObject.price}</h3></Col>
                            </Row>
                            <Row></Row>
                            <Row className='select-size-text'><Col>Please select your size</Col></Row>
                            
                            <Row className='size-selector-group'>
                                <Col className='size-selector-col'>
                                    <ButtonGroup className='individual-size-selector' fluid>
                                        {productObject.sizes.length>3 ? productObject.sizes.slice(0,3).map(el => <Button variant='outline-dark' className='size-button'>{el}</Button>):

                                        productObject.sizes.map(el => <Button variant='outline-dark' className='size-button'>{el}</Button>)
                                    }
                            ...
                            <Row className='size-selector-group'>
                                <Col className='size-selector-col'>
                                    <ButtonGroup className='individual-size-selector' fluid>
                                        {productObject.sizes.length>3 ? productObject.sizes.slice(3,productObject.sizes.length).map(el => <Button variant='outline-dark' className='size-button'>{el}</Button>):
                            ...
                            <Row className='buy-button-row'>
                                <Col><Button variant='dark' className='buy-button'>Buy now</Button></Col>
                            </Row>                       
                   </Col>
                   </Container>                
        </div>
